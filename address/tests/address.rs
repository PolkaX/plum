...
#[test]
fn test_decode() {
    for a in ALL_TEST_ADDRESS.iter() {
        let addr = Address::decode(a).unwrap();
        assert_eq!(*a, addr.display(Network::Test).unwrap())
    }
}
...
fn new_secp256k1_address_should_work() {
    ...
    for (b, s) in test_cases.iter() {
        ...
        assert_eq!(s.to_string(), addr.display(Network::Test).unwrap());
    }
}
...
fn test_random_id_address() {
    ...
    let (id, _) = unsigned_varint::decode::u64(&keypair.pubkey).unwrap();
    let addr: Address = Account::ID(id).try_into().unwrap();
    let mut buf = unsigned_varint::encode::u64_buffer();
    let id_c = unsigned_varint::encode::u64(id, &mut buf).to_vec();
...
fn test_random_generate_address() {
    ...
    let addr: Address = Account::SECP256K1(keypair.pubkey.clone())
        .try_into()
        .unwrap();
    let new_addr = Address::decode(&addr.display(Network::Test).unwrap()).unwrap();
...
fn test_invalid_string_address() {
    let test_cases = vec![
        ...
        ("t0banananananannnnnnnn", AddressError::InvalidID),
