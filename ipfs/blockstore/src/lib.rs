...
pub trait BlockStore: ToBatchDataStore {
    ...
    fn put_many(&mut self, blocks: &[Block]) -> io::Result<()> {
    ...
    fn delete(&mut self, cid: &Cid) -> io::Result<()> {
        let key = multihash_to_datastore_key(cid.hash().as_bytes());
